  ...
  return (
    <Show isLoading={isLoading}>
      <Descriptions column={1} bordered title={t("relation.people")}>
        ...
        <Descriptions.Item label={t("people.name")}>{record?.name}</Descriptions.Item>
        <Descriptions.Item label={t("people.calledName")}>{record?.calledName || "-"}</Descriptions.Item>
        <Descriptions.Item label={t("people.status")}>{record?.status || "-"}</Descriptions.Item>
        <Descriptions.Item label={t("people.birthday")}>
          {record?.birthday ? new Date(record.birthday).toLocaleDateString() : "-"}
        </Descriptions.Item>
        <Descriptions.Item label={t("people.note")}>{record?.note || "-"}</Descriptions.Item>
      </Descriptions>
    </Show>
  );
};
